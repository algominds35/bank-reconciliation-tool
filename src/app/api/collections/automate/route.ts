...
export async function POST(req: NextRequest) {
  try {
    console.log('🚀 Starting automated collections process...')
    
    // Get overdue invoices from standalone invoice system
    ...
    if (!invoices || invoices.length === 0) {
      console.log('✅ No invoices found for collections')
      return NextResponse.json({ 
        success: true, 
        message: 'No invoices found for collections',
        emailsSent: 0
      })
    }
    ...
    for (const invoice of invoices) {
      try {
        ...
        if (success) {
          ...
          // Record the reminder in collection_reminders table
          await supabase
            ...
            .insert({
              ...
              phase: phase,
              email: client.email,
              subject: `${phase.charAt(0).toUpperCase() + phase.slice(1)} Reminder - Invoice #${invoice.invoice_number}`,
              content: `Payment reminder sent for invoice #${invoice.invoice_number}`,
              status: 'sent'
            })
        } else {
          emailsFailed++
    ...
    console.log(`✅ Automated collections process complete. Sent: ${emailsSent}, Failed: ${emailsFailed}`)
    ...
    console.error('❌ Automated collections error:', error)
    return NextResponse.json({ 
      success: false, 
